...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	public Decreaser<T> insert(T thing) {
		...
		//System.out.println("its loc is"+array[size].loc);
		//
	...
	public void moveItem(int from, int to){
		Decreaser<T> a = array[from];
		Decreaser<T> b = array[to];
		array[to] = a;
		array[to].loc = to;
		array[from] = b;
		array[from].loc = from;
...
//		}
		...
		//System.out.println(HeapToStrings.toTree(this));
		//System.out.println("The root is "+array[1].getValue());
		return ans;
	}
		...
		if(where*2<=size){
			if(where*2+1<=size){
				if(array[2*where].getValue().compareTo(array[2*where+1].getValue())>0){
					if(array[where].getValue().compareTo(array[2*where+1].getValue())>0){
...
//						Decreaser<T> b = array[where*2+1];
//						array[where*2+1] = a;
//						array[where*2+1].loc = where*2+1;
//						array[where] = b;
//						array[where].loc = where;
//						this.decrease(where);
//						this.heapify(where*2+1);
//						this.heapify(where*2);
//						ticker.tick();
//					}
//					else{
//						Decreaser<T> a = array[where];
//						Decreaser<T> b = array[where*2];
//						array[where*2]= a;
