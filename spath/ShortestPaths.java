...
public class ShortestPaths {
	...
	public void run() {
		...
		do{
			...
			int d=u.getDistance();
			for(Iterator<Edge> iter =u.getVertex().edgesFrom().iterator();iter.hasNext();){
				Edge e = iter.next();
				VertexAndDist v = map.get(e.to).getValue();
				if(weights.get(e)+d<v.getDistance()){
					map.get(v.getVertex()).decrease(v.sameVertexNewDistance(weights.get(e)+d));
					toEdge.put(v.getVertex(), e);
				}
			}
		}while(!pq.isEmpty());
	}
	...
	public LinkedList<Edge> returnPath(Vertex endVertex) {
		...
		// FIXME
		//
        while(toEdge.get(endVertex) != null){
